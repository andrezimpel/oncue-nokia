import * as styles from './snake.module.scss'
...
class SnakeGame extends React.Component {
  ...
  componentDidMount() {
    ...
    document.getElementById('btnTop').addEventListener('click', this.handleKeyDown)
    document.getElementById('btnLeft').addEventListener('click', this.handleKeyDown)
    document.getElementById('btnRight').addEventListener('click', this.handleKeyDown)
    document.getElementById('btnBottom').addEventListener('click', this.handleKeyDown)
    this.gameLoop()
  }

  componentDidUnMount() {
    window.removeEventListener('keydown', this.handleKeyDown)
    document.getElementById('btnTop').removeEventListener('click', this.handleKeyDown)
    document.getElementById('btnLeft').removeEventListener('click', this.handleKeyDown)
    document.getElementById('btnRight').removeEventListener('click', this.handleKeyDown)
    document.getElementById('btnBottom').removeEventListener('click', this.handleKeyDown)
  }

  initGame() {
    // Game size initialization
    let percentageWidth = this.props.percentageWidth || 102
    let width =
  ...
  handleKeyDown(event) {
    if (event.keyCode === undefined) {
      if (event.target.id === 'btnLeft') event.keyCode = 37
      if (event.target.id === 'btnTop') event.keyCode = 38
      if (event.target.id === 'btnRight') event.keyCode = 39
      if (event.target.id === 'btnBottom') event.keyCode = 40
    }
    // if spacebar is pressed to run a new game
    if (this.state.isGameOver && event.keyCode === 67) {
      this.resetGame()
      return
    }
